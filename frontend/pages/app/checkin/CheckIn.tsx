...
import { dayParams, tzOffset, useDatePicker } from "../../../hooks/useDatePicker";
import { ROLE_LABELS, useAuth } from "../../../context/AuthContext";
import ActionError from "../../../components/ActionError";
import { api, useCreateHandoff } from "../../../hooks/useApi";
...
export default function CheckIn() {
  ...
  // Who the operatory has been told about. Local, but no longer the
  // whole story: it mirrors a handoff that really was written, and is
  // only set once the POST comes back.
  const [notified, setNotified] = useState<Set<string>>(new Set());
  // Per patient, persistent. Both writes on this screen already set
  // their local state inside onSuccess — the gap was that a failure
  // only ever appeared in a toast.
  ...
  const checkInMutation = useMutation({
    mutationFn: async (p: CheckInPatient) => {
      await api.post("/checkin", {
        ...
        // checkin_day used to come from CURRENT_DATE — UTC — so a
        // patient checked in at 8:30pm Eastern was recorded as
        // arriving tomorrow and left the day the desk was looking at.
        tz_offset: tzOffset(),
      });
    },
    onSuccess: (_r, p) => {
      void queryClient.invalidateQueries({ queryKey: ["checkin", "today"] });
      clearError(p.pred_request_id);
    ...
    onError: (_e, p) =>
      setCardErrors((prev) => ({
        ...prev,
        [p.pred_request_id]:
          "Not checked in. The rest of the practice still shows this " +
          "patient as not arrived.",
      })),
  });
  ...
  function notify(p: CheckInPatient) {
    // A real handoff now, not a set of ids in this tab. The note names
    // the alerts the desk is flagging, because "patient is ready" on
    // its own tells the clinician nothing they cannot see from the
    // schedule.
    const flags = p.alerts?.length
      ? ` Flags: ${p.alerts.map((a) => a.title).join("; ")}.`
      : "";
    handoff.mutate(
      {
        ...
        to_role: "dentist",
        note:
          `${p.patient_name} is checked in and waiting in reception ` +
          `(${p.appointment_time}, ${p.procedure_summary}).${flags}`,
      },
      {
        onSuccess: () => {
        ...
        onError: () =>
          setCardErrors((prev) => ({
            ...prev,
            [p.pred_request_id]: `${p.provider_name} was NOT notified. The note did not send.`,
          })),
      },
    );
  }

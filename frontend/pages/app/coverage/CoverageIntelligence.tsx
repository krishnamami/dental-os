...
import { useRequestDocuments } from "../../../hooks/useApi";
...
export default function CoverageIntelligence() {
  ...
  const [docsFor, setDocsFor] = useState<{ id: string; name: string } | null>(
    null,
  );
  ...
  /** Really posts. The id was on the row all along — this held only
   *  the patient's name, which is why it could not do anything. */
  async function sendDocs(
    ...
    setDocsFor(null);
    setToast(
      `${documents.length} document request${documents.length === 1 ? "" : "s"} sent for ${row.name} ✓`,
    );
    ...
    <div>
          ...
          <Section title={`Need my action (${ACTION.length})`}>
            {ACTION.map((p) => (
              <PatientCard
                ...
                onRequestDocs={() => setDocsFor({ id: p.id, name: p.name })}
              />
            ))}
          </Section>
                  ...
                  <tr key={r.id} className="hover:bg-accord-green-50/40">
                    ...
                    <td className="px-4 py-2.5">
                      <div className="flex justify-end gap-1.5">
                        ...
                        <button
                          type="button"
                          onClick={() => setDocsFor({ id: r.id, name: r.name })}
                          className="rounded-lg border border-gray-300 px-2.5 py-1 text-[11.5px] font-medium text-gray-700 transition hover:bg-gray-50"
                        >
                          Request docs
                        </button>
      ...
      {docsFor && (
        <RequestDocsModal
          ...
          onSend={(documents, note) => sendDocs(docsFor, documents, note)}
        />
      )}

...
import SubmissionQueue, {
  isReadyToSubmit,
} from "../../../components/SubmissionQueue";
    ...
    queryFn: async () =>
      (
        await api.get<QueueRow[]>(
          `/decisions/queue?${dayParams(selectedDate)}`,
        )
      ).data,
    staleTime: 60_000,
  });
  const rows: QueueRow[] = Array.isArray(data) ? data : [];
  // isReadyToSubmit, not r.submission_ready: an engine-clear case that
  // no clinician has signed counts as blocked on BOTH the stat card and
  // the list below it. Two filters that disagreed would put "3 ready"
  // above a list of one.
  const readyCount = rows.filter(isReadyToSubmit).length;
  const blockedRows = rows.filter((r) => !isReadyToSubmit(r));

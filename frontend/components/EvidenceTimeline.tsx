...
export interface TimelineNode {
  ...
  signal?: Signal;
  /**
   * The stored document behind this node, if there is one.
   *
   * ⚠ THIS FIELD REPLACES A LEAK. `detail` used to be set to the
   * radiograph's s3_key, so the caption under "PA X-ray" read
   * `suwanee_smiles/DA-A01/DA-A01_PA_XRAY_TOOTH19.pdf` — an internal
   * object path printed to a clinician, promising a document that
   * nothing could open. The id resolves through the presigning
   * endpoint instead, and the path never reaches the browser.
   */
  ...
  if (xray || boneLoss !== undefined) {
    nodes.push({
      ...
      sub: xray
        ...
        : "Radiograph on file",
      documentId: xray?.evidence_id ?? undefined,
    });
  }
...
export default function EvidenceTimeline({
  ...
  /** Needed to resolve a document: the endpoint is scoped by pre-D. */
      ...
      {nodes.map((node, i) => {
        ...
        return (
          <li key={node.id} className="relative flex gap-3">
            ...
            {/* Outside the <button>, not inside it: a button nested in a
                button is invalid, and the browser would give the click
                to whichever it felt like. */}
            {node.documentId && predRequestId && (
              <span className="absolute bottom-0 right-2">
                <DocumentLink
                  predRequestId={predRequestId}
                  evidenceId={node.documentId}
                  label="Open document"
                />
              </span>
            )}

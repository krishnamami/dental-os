 ...
 *
 * ⚠ THE FALLBACK MATTERS MORE THAN THE MAP. This dropped through to
 * the raw value for anything unlisted, and "waiting_period" rendered
 * with its underscore next to "Bundling conflict" — the two listed
 * neighbours only looked right because they are single words. Unknown
 * reasons now get de-underscored rather than shown as an enum.
 */
...
export default function RevOpsAnalytics() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Five cards since Denied split in two. */}
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        />
        {/* TWO CARDS, NOT ONE. These were "1 payer · 7 engine" in a
            single value. The comment below has always said they must
            not be added together — and then the card put them side by
            side in one number, which is an invitation to add them.
            13 + 20 + 1 + 7 = 41 against 40 cases, every time. */}
        <Card
          label="Engine denied"
          value={String(an?.cases.denied ?? "—")}
          note="predicted before sending · part of the 40"
          tone="text-accord-amber-900"
        />
        <Card
          label="Payer denied"
          value={String(an?.denials.total ?? "—")}
          note="a payer actually refused · not a separate case"
          tone="text-accord-amber-900"
        />
      </div>

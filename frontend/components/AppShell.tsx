...
export default function AppShell() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* ── #0B1220, per the brand spec ─────────────────────────────
          This was white, and the contrast note that used to live here
          is still the governing constraint — it is just satisfied the
          other way round now. The problem it fixed was slate-400/500
          greys at ~2.8:1 on white; white and white/75 on #0B1220 are
          ~18:1 and ~11:1, so the practice name and the signed-in
          identity are further above the AA floor than before, not
          nearer it.

          One header serves every role and every screen, so this is not
          a /coverage change — Sarah, Jennifer, Kim, Dr. Chinta and Dr.
          Shyam all get it. */}
      <header className="sticky top-0 z-30 flex min-h-[52px] items-center justify-between gap-3 border-b border-white/10 bg-[#0B1220] px-4 py-2 sm:px-6">
        <Link to={demoLink("/")} className="flex min-w-0 flex-col">
          <AccordLogo size={24} dark />
          <span className="mt-0.5 truncate text-[12px] text-white/75">
            {tenantName}
          </span>
        </Link>
        ...
        <div className="flex flex-shrink-0 items-center gap-2">
          ...
          {!isDemo && (
            <button
              ...
              // The ghost button from the spec: rgba(255,255,255,0.12)
              // on the navy, with a hairline border so it is still a
              // button on a monitor that crushes the blacks.
              className="rounded-lg border border-white/20 bg-white/[0.12] px-3 py-1 text-sm text-white transition hover:bg-white/20 max-[899px]:min-h-[44px]"
            >
              Sign out
            </button>
          )}

        ...
        <h3 className="mt-1 text-[15px] font-semibold text-gray-900">
          {cited
            ? [
                ...
                // The payer's NAME, not a document reference — it is
                // the same string on all 50 library rows.
                condition.payer_citation && `payer: ${condition.payer_citation}`,
              ]
                .filter(Boolean)
                .join(" · ")
    ...
    >
      ...
      <div className="flex flex-shrink-0 flex-wrap items-center gap-2 sm:flex-col sm:items-end">
        ...
        <div className="flex flex-wrap gap-1.5">
          <button
            type="button"
            disabled={mutation.isPending || resolved || !canFeedback}
            title={
              canFeedback
                ? "Records feedback. The condition clears when the pre-D is re-run with the evidence in place."
                : NO_ENGINE_FEEDBACK
            }
            onClick={() => {
              setResolved(true);
